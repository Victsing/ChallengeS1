<template>
  ...
  <v-dialog v-model="premiumDialog" persistent max-width="680">
    <v-card>
      <v-card-title class="bg-blue text-center"> Informations de paiement utilisateur </v-card-title>
      <v-card-text>
        <v-row>
          <v-col cols="12">
            <v-list-subheader class="grey--text text--lighten-1 pl-0 subheader">Name</v-list-subheader>
            <v-text-field single-line outlined label="Name" v-model="name" />
          </v-col>
        </v-row>
        <v-row>
          <v-col cols="12">
            <v-list-subheader class="grey--text text--lighten-1 pl-0 subheader">Email</v-list-subheader>
            <v-text-field single-line outlined label="Email" v-model="email" />
          </v-col>
        </v-row>
        <v-row>
          <v-col cols="12">
...
onMounted(async () => {
  stripe.value = await loadStripe(import.meta.env.VITE_STRIPE);
